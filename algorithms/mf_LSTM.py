...
def lstm(df):
    days = 15  # Standardized prediction length to match ARIMA
    
    # Ensure we have enough data
    if len(df.dropna()) < 100:
        raise ValueError("Insufficient historical data for LSTM prediction")
    
    df1 = df['Close'].dropna()  # Remove NaN values for training
